...
import { Users, Clock, MapPin, BookOpen, MessageCircle, Plus, Edit, Eye, ArrowLeft, Zap, UserCheck } from "lucide-react";
...
const AdminDashboard = () => {
  ...
  const { potentialGroups, runAutoMatching, isRunningAutoMatching } = useAutoMatching();
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-6 mb-8">
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-2xl font-bold">{potentialGroups.length}</p>
                  <p className="text-gray-600">Auto-Match möglich</p>
                </div>
        ...
        <Card className="mb-8">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              Automatisches Gruppen-Matching
            </CardTitle>
            <CardDescription>
              Automatisch Gruppen erstellen basierend auf Verfügbarkeiten und Kriterien
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-sm text-gray-600 mb-2">
                  {potentialGroups.length} potentielle Gruppen gefunden
                </p>
                <Button
                  onClick={() => runAutoMatching.mutate()}
                  disabled={isRunningAutoMatching || potentialGroups.length === 0}
                  className="flex items-center gap-2"
                >
                  <Zap className="h-4 w-4" />
                  {isRunningAutoMatching ? "Wird ausgeführt..." : "Auto-Matching starten"}
                </Button>
              </div>
            </div>
            ...
            {potentialGroups.length > 0 && (
              <div className="mt-4">
                <h4 className="font-medium mb-2">Potentielle Gruppen:</h4>
                <div className="space-y-2">
                  {potentialGroups.slice(0, 3).map((group, index) => (
                    <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      <div>
                        <span className="font-medium">{group.bundesland} - {group.klassenstufe}</span>
                        <span className="text-sm text-gray-600 ml-2">
                          ({group.student_count} Schüler, {group.common_time_slots.length} gemeinsame Zeiten)
                        </span>
                      </div>
                    </div>
                  ))}
                  {potentialGroups.length > 3 && (
                    <p className="text-sm text-gray-600">
                      ... und {potentialGroups.length - 3} weitere
                    </p>
                  )}
                </div>
              </div>
      ...
      <TabsContent value="active">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Übersicht aller aktiven Gruppen mit WhatsApp-Links
            </CardDescription>
          </CardHeader>
          <CardContent>
            {activeGroups.length === 0 ? (
